...
class EvaluationCriteria(object):
    ...
    def get_kappa(self, stats, index, y_true, y_pred, labels=None):
        """
        Calculates the kappa score
        :param stats: the stats dataframe
        :param index: row index in the stats dataframe
        :param y_true: the true labels
        :param y_pred: the predicted labels
        ...
        """
        if self.kappa:
            stats.loc[index, (const.summary_level, const.kappa)] = metrics.cohen_kappa_score(y_true, y_pred,
                                                                                             labels=labels)
    ...
    def evaluate(self, index, y_true, y_predict, y_predict_proba) -> pd.DataFrame:
        ...
        self.get_kappa(stats, index, y_true, y_predict, labels)
